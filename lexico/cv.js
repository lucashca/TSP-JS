...
function confirmarClick(){
    var val = document.getElementById("qtdNodes").value;
...
function draw(){
    ...
    comb = getCombination(nodes);
    bestComb = getCombination(bestRoutes);
    fill(255);
    var percent = 100 * (count / totalPermutations);
    text("Interação "+count+" de "+totalPermutations+"     "+nf(percent, 0, 2) + "% completed       Menor distância "+ bestWeight, 20, height-10);
    text("Combinação "+ comb+"     Melhor combinação "+ bestComb, 20, height+30);
